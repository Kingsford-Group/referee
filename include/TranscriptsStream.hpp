...
class TranscriptsStream {
	...
	shared_ptr<string> readTranscriptSequence(string const & ref_name, FaiEntry & entry) {
		...
		// // eat up trailing newlines
		// while ( !isalpha( S.back() ) ) {
		// 	S.pop_back();
		// }

		// int i = S.size() - 1, prev_i = i;
		// string str;
		// while (i >= 0) {
		// 	if ( isalpha(S[i]) ) {
		// 		i--;
		// 	}
		// 	else {
		// 		// we see the first of the newlines
		// 		for (auto it = S.rbegin(); it != S.rbegin() + (prev_i - i) && it != S.rend(); it++)
		// 			str.push_back( *it );
		// 		// pop these chars off, including any new lines we expect to see
		// 		for (auto j = 0; j < (prev_i - i + newline_chars) && S.size() > 0; j++)
		// 			S.pop_back();
		// 		i -= newline_chars;
		// 		prev_i = i;
		// 	}
